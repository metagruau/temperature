...
mkdir -p $ROOT_WWW/archives-yearly-inside

# leave explicitly blank to force graph generation when script is launched
TODAY=""
...
while true; do
    ...
    # generate and archive the daily graphs for yesterday if they don't exist
    ...
    # generate inside temperatures over the current year
    if [ "$TODAY" != "$(date +%d)" ]; then
        TODAY=$(date +%d)

        THISYEAR=$(date +%Y)
        DATE_MIN="${THISYEAR}0101T0000"
        DATE_MAX="${THISYEAR}1231T2359"
        extract-data.py --date-min "$DATE_MIN" --date-max "$DATE_MAX" < inside.txt \
            | reduce-min-max-temp.awk > yearly-min-max-inside-temperature.txt
        plot-min-max-yearly-temperature.sh yearly-min-max-inside-temperature.txt $THISYEAR > $ROOT_WWW/archives-yearly-inside/$THISYEAR-temperature.svg
    fi
